...
久しぶりに。完璧を目指したら先に進まないので、とりあえず手ゴネで配置だけは納得いくものを作って、それで練習して不満点を揃えて改良...みたいな感じでいこう。

アドカレ版をベースに、配置が変なところを直していこう...と思ったけど、まぁいいか。連節とか考えられてるのであれば変に動かして損しそうだから。

なので、とりあえず現状のバージョンで練習してみよう。GO!

### 2025-12-16

うーん、ぁが上段にあるのがちょっと気になるんだけど、とりあえずこれでいいかなぁ。やっぱり納得できないので、条件を入れてQWERTGBには入れないようにしてもらう。

いつまでたっても終わらないのでとりあえずこれでいこう。ょシフトの左手中段に全然配置されていないのが気になるけど...。次は配列図を作ります。
...
配置順を変更してスコアが少し向上。最終的に置くところがなくなってそうな感じがあったので（へとか）、濁音にも外来音にも拗音にもならないかなで、単打にならないものは後から配置するように変更してみた。

次は、中段に拗音を置かないように探索を変更してみる。そうすると、シフト面に中段に置けるので少し良くなる気がする。
...
アドカレで今できているところまで公開する予定なので、もっともスコアが高かったものを調整していこうと思います。

濁音は両方シフトできるからいいとして、拗音、半濁音、外来音の打ちやすさは確認しておきたいですね。あとは、句読点がいい場所に配置されているか、とか。

今シフトキーをコストに含めてないので、打ちづらい場所に入っている可能性がいっぱいあります。

kouyさんのシート見ながら調整するのがいいかな。

試してみたところ、「ぺ」「しぇ」「みゃみゅみょ」あたりが難しかった。他は割といい感じ。あと、シフト面の配置をもっといい感じにしたい。中段に拗音を置いてしまうとホームポジションシフト面に置けなくなってしまうから、それ以外の場所に置くことを考えてみよう。

あとは、濁音になるかなと拗音になるかなを同じ位置に配置しないルールはあった方がいいので追加する。「こ」「み」のところ。なぜかというと、ゃ後置で濁音を打てるようにしいので。「こゃ」で「ご」にできない。まぁ今は打ちづらいqsに配置されているから使うことないかもしれないけど、気づいたので修正しておく。

### 2025-12-13

なんかビーム幅増やしたのにスコア落ちるからおかしいな？と思ったら、デバッグ用にtrigramを上位5件だけ入れるようにしてたのをわすれてた...。それでビーム幅増やしても早かったのか。

3-gram 4000件、ビーム幅100で1分くらい。やっと貪欲法を超えたよ！
...

{
  "score": 671828,
  "kpm": 857.35,
  "totalSeconds": 97731979,
  "totalCount": 1094317502
}
```


ビームサーチ実装できたのでビーム幅500で回してみた。前までの結果はスコア計算がバグってたので消しておいた。まだ貪欲より低いのでどうすればいいか。
