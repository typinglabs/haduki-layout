...
- JISキーボードでの使用を主に想定する。
- 3行10列の30キーに収める。
- 普段使いとタイピングゲームの両方での使用を想定する。
- 濁音同置、半濁音同置、拗音同置、外来音同置でシンプルにする。
...
拗音、濁音、半濁音、外来音を入力で後置シフトに配置されているかなを使う場合は、後置シフトを省略して入力します。

例）

- 拗音の後置シフト省略
  - Shift + る(u) = ち のとき、る(u) + ゃ = ちゃ
- 濁音の後置シフト省略
  - い(;) + ゅ(d) = ほ のとき、い(;) + ゛(l) = ぼ
- 半濁音の後置シフト省略
  - い(;) + ゅ(d) = ほ のとき、い(;) + ゜(k) = ぽ
- 外来音のシフト省略
  - な(g) + ゅ(d) = ふ のとき、Shift + な(g) = ふ

また、例外がいくつかあります。

- 濁音にも拗音にもなるかな（「き」「し」「ち」「ひ」）を濁音化する場合は、「゛」キーを使用します。
- 「ぴ」は「ひ」+「よ」で入力できないため、代わりに「は」+「ゅ」で入力します。
- 単打ではないイ段かな（「ひ」「み」「り」）を単体で入力する場合は、通常のシフトを使います。

Shiftキーの面の空いている箇所は単打と同じキーが配置されているので、シフトキーを押すタイミングはある程度融通が利くようになっています。例えば「てぃ」は、「て + Shift(い)」の他に「Shift(て + い)」でも入力できます。
